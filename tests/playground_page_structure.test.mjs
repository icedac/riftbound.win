...
test("playground renders Hearthstone-style seats with card images and hover preview", async () => {
  ...
  assert.match(js, /data-zone-role/);
  ...
  assert.match(js, /function tableTitleText/);
  ...
  assert.match(css, /\.zone-metadata/);
  assert.match(css, /\.zone-empty/);
  assert.match(css, /\.card-hover-preview\s*\{[^}]*position: absolute;/s);
  assert.match(css, /\.card-hover-preview\.is-empty/);
  ...
  assert.match(css, /\.playground-lobby\s*\{[^}]*max-height: calc\(100svh - 78px\)/s);
  assert.match(css, /\.playground-side\s*\{[^}]*max-height: calc\(100svh - 78px\)/s);
  assert.match(css, /\.table-actions,[^}]*\.zone-grid,[^}]*\.seat-hud\s*\{[^}]*min-width: 0;/s);
  assert.match(css, /\.selected-card-tools\s*\{[^}]*grid-template-columns: repeat\(auto-fit, minmax\(96px, 1fr\)\)/s);
  assert.match(css, /grid-template-columns:\s*minmax\(82px, 0\.48fr\) minmax\(110px, 0\.62fr\) minmax\(0, 1\.25fr\)/);
